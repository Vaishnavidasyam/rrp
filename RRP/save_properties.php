...
foreach ($properties as $property) {
    ...
    if ($res->num_rows === 0) {
        // Insert new
        $sql = "INSERT INTO display (id, title, description, city, state, price, images, google_maps_url, name, email, saved_to_display, created_at) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("issssisssss", 
            ...
            $name, $email, $property['created_at']);
        $stmt->execute();
    } else {
        // Update existing
        $sql = "UPDATE display 
                SET title = ?, description = ?, city = ?, state = ?, price = ?, images = ?, google_maps_url = ?, name = ?, email = ?, saved_to_display = 1, created_at = ?
                WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssissssi", 
            ...
            $name, $email, $property['created_at'], $property['id']);
        $stmt->execute();
    }
    $stmt->close();
}
